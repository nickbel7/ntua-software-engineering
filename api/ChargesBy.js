...
router.get('/:op_ID/:date_from/:date_to', function(req, res) {
	...
	pool.connect(function(err, client) {
		...
		client.query("SELECT provider2_name AS provider_name, COUNT(charge) AS total_passes, cast(SUM(charge) as DECIMAL(10,2)) AS total_charge FROM passes_transposed WHERE provider1_id = $1 AND provider1_id!=provider2_id AND pass_time BETWEEN $2 AND $3 group by provider2_name order by provider2_name", [op_ID, date_from, date_to], function(err, result) 
		{
			if(err) {
