...
router.get('/:station_id/:date_from/:date_to', function(req, res) {
	...
	pool.connect(function(err, client) {
		...
		var station_op;
                client.query("SELECT DISTINCT provider1_name FROM passes_transposed inner join stations on stations.station_name_abbr=passes_transposed.station_name WHERE station_id = $1", [station_id], function(err, result) {
                        if(result.rows.length)
                                station_op = result.rows[0].provider1_name;
                });
		client.query("SELECT ROW_NUMBER() OVER (ORDER BY 1), pass_code, pass_time, vehicle_code, provider2_name, pass_type, charge FROM passes_transposed inner join stations on stations.station_name_abbr=passes_transposed.station_name WHERE station_id = $1 AND pass_time BETWEEN $2 AND $3", [station_id, date_from, date_to], function(err, result) {
			if(err) {
